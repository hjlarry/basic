...
* 将指针赋值给0或指针与0之间的比较运算
其他形式的指针运算都是非法的。
指针和整数的加减运算，实际上看指针的类型，比如p+n，p指向的是一个short类型，short可能占2字节，那么计算时本质上就是p+2n
指针的减法运算，如果p、q都是某个数组的不同元素，且p<q，那么q-p+1就是p和q之间的元素数目
*/
...
/*
六、指针数组
使用场景举例：利用指针数组排序文本行，避免移动其本身带来的大量开销，见练习5-7

二维数组其实是特殊的一维数组，它的每个元素也是一个一维数组。那么在函数声明中需要确定数组的列数，而行数就无所谓了。例如:
f(int daytab[2][13]){...}
也可以写成 f(int daytab[][13]){...}
相当于 f(int (*daytab)[13]){...}

指针数组和二维数组是容易混淆的两个概念，例如二维数组a和指针数组b：
int a[5][10];
int *b[10];
如果b中每个元素的指针也指向一个数组，那么a[3][4]和b[3][4]在语法角度都是合法的。但在内存角度a的内存布局是固定的，b就可以存储不同长度的字符串

七、指向函数的指针
可以定义指向函数的指针，这种指针可以被赋值，存放在数组中，传递给某个函数或者作为函数的返回值。见 5-7.c

C语言的声明有时候很容易混淆，例如:
int *f();   f表示一个函数，返回整数类型的指针
int (*pf)();    pf表示一个指向函数的指针，整个函数返回整型
*/
